...
def info():
    ...
    """
    cellInfo = { 'name' : 'DFEQQN_DL',
                 'pinList' : ['D', 'E', 'CLK', 'Q', 'QN', 'VDD', 'VSS'],
                 'description' : 'D flip-flop with enable and Q and QN outputs with weak drive strength'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    DFEQQN_DL = Symsubcircuit(cellInfo['name'], ['D', 'E', 'CLK', 'Q', 'QN', 'VDD', 'VSS'])
    # Clock inverters
    ...
    # Slave latch
    invz_ms = invz_par("invz_ms", ['di', 'clki', 'clkn', 'qin', 'VDD', 'VSS'], {'WN' : 400e-9, 'WP' : 600e-9})
    inv_sl = inv_par("inv_sl", ['qin', 'qi', 'VDD', 'VSS'], {'WN' : 200e-9, 'WP' : 300e-9})
    ...
    DFEQQN_DL.addElement([inv_clkn, inv_clki, inv_enb,
                          n0, n1, n2, n3, n4,
                          p0, p1, p2, p3, p4,
                          inv_ml, invz_mfb, 
                          invz_ms, 
    ...
    # Flatten the circuit
    if genFlat:
        DFEQQN_DL_flat = DFEQQN_DL.flat()
    if anonimize:
        DFEQQN_DL_flat.anonimize()
...
def check(verbose = False):
    ...
    spec = { 'D' : 'D', 'EN' : 'E', 'Q' : 'Q', 'QN' : 'QN', 'CLK' : 'CLK' }
    res = sim.dffCheck(spec)

    if verbose:
        # Plot waveforms
        wave = sim.filterResults( ['D', 'E', 'CLK', 'Q', 'QN'] )
