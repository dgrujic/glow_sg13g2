...
def info():
    ...
    """
    cellInfo = { 'name' : 'DFEQN_D1',
                 'pinList' : ['D', 'E', 'CLK', 'QN', 'VDD', 'VSS'],
                 'description' : 'D flip-flop with enable and QN output with drive strength x1'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    DFEQN_D1 = Symsubcircuit(cellInfo['name'], ['D', 'E', 'CLK', 'Q', 'QN', 'VDD', 'VSS'])
    # Clock inverters
    ...
    DFEQN_D1.addElement([inv_clkn, inv_clki, inv_enb,
                          n0, n1, n2, n3, n4,
                          p0, p1, p2, p3, p4,
                          inv_ml, invz_mfb, 
                          invz_ms, 
    ...
    # Flatten the circuit
    if genFlat:
        DFEQN_D1_flat = DFEQN_D1.flat()
    if anonimize:
        DFEQN_D1_flat.anonimize()
...
def check(verbose = False):
    ...
    spec = { 'D' : 'D', 'EN' : 'E', 'QN' : 'QN', 'CLK' : 'CLK' }
    res = sim.dffCheck(spec)

    if verbose:
        # Plot waveforms
        wave = sim.filterResults( ['D', 'E', 'CLK', 'QN'] )
