...
def info():
    ...
    """
    cellInfo = { 'name' : 'DFERQ_D2',
                 'pinList' : ['D', 'E', 'RN', 'CLK', 'Q', 'VDD', 'VSS'],
                 'description' : 'D flip-flop with enable, reset and Q output with drive strength x2'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    DFERQ_D2 = Symsubcircuit(cellInfo['name'], ['D', 'E', 'RN', 'CLK', 'Q', 'VDD', 'VSS'])
    # Clock inverters
    ...
    DFERQ_D2.addElement([inv_clkn, inv_clki, inv_enb,
                          ns, n0, n1, n2, n3, n4,
                          ps, p0, p1, p2, p3, p4,
                          inv_ml, invz_mfb, 
                          invz1_ms, invz2_ms,
    ...
    # Flatten the circuit
    if genFlat:
        DFERQ_D2_flat = DFERQ_D2.flat()
    if anonimize:
        DFERQ_D2_flat.anonimize()
...
def check(verbose = False):
    ...
    spec = { 'D' : 'D', 'EN' : 'E', 'CLRN' : 'RN', 'Q' : 'Q', 'CLK' : 'CLK' }
    res = sim.dffCheck(spec)

    if verbose:
        # Plot waveforms
        wave = sim.filterResults( ['D', 'E', 'RN', 'CLK', 'Q'] )
