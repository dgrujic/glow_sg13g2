...
def info():
    ...
    """
    cellInfo = { 'name' : 'OR4_DL',
                 'pinList' : ['A', 'B', 'C', 'D', 'Y', 'VDD', 'VSS'],
                 'description' : 'OR4 with weak drive strength'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    OR4_DL = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    wn = 190e-9
    wp = 3 * wn
    ...
    OR4_DL.addElement([nor4_inst, inv_inst])

    # Flatten the circuit
    if genFlat:
        OR4_DL_flat = OR4_DL.flat()
    if anonimize:
        OR4_DL_flat.anonimize()
