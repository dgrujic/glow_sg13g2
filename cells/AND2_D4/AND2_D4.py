...
def info():
    ...
    """
    cellInfo = { 'name' : 'AND2_D4',
                 'pinList' : ['A', 'B', 'Y', 'VDD', 'VSS'],
                 'description' : 'AND2 with drive strength x4'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    AND2_D4 = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    wn = 1000e-9
    wp = 1100e-9
    ...
    inv_ngn = 2
    inv_ngp = 2
    inv_inst = inv_par('inv_inst', ['NO', 'Y', 'VDD', 'VSS'], {'WN' : inv_ngn*inv_wn, 'WP' : inv_ngp*inv_wp, 'NGN' : inv_ngn, 'NGP' : inv_ngp})
    AND2_D4.addElement([nand2_inst, inv_inst])

    # Flatten the circuit
    if genFlat:
        AND2_D4_flat = AND2_D4.flat()
    if anonimize:
        AND2_D4_flat.anonimize()
