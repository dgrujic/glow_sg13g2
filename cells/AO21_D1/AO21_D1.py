...
def info():
    ...
    """
    cellInfo = { 'name' : 'AO21_D1',
                 'pinList' : ['A', 'B', 'C', 'Y', 'VDD', 'VSS'],
                 'description' : 'AO21 with drive strength x1'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    AO21_D1 = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    wns = 300e-9
    wnp = 200e-9
    wp = 400e-9
    ...
    AO21_D1.addElement([n0, n1, n2, p0, p1, p2, inv_inst])

    # Flatten the circuit
    if genFlat:
        AO21_D1_flat = AO21_D1.flat()
    if anonimize:
        AO21_D1_flat.anonimize()
