...
def info():
    ...
    cellInfo = { 'name' : 'INV_D14',
                 'pinList' : ['A', 'Y', 'VDD', 'VSS'],
                 'description' : 'Inverter with drive strength x14'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    INV_D14 = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    inv14 = inv_par('inv14', ['A', 'Y', 'VDD', 'VSS'], {'WN' : wn, 'WP' : wp, 'NGN' : ngates, 'NGP' : ngates})
    INV_D14.addElement(inv14)

    # Flatten the circuit
    if genFlat:
        INV_D14_flat = INV_D14.flat()
    if anonimize:
        INV_D14_flat.anonimize()
