...
def info():
    ...
    """
    cellInfo = { 'name' : 'DFASQN_D1',
                 'pinList' : ['D', 'CLK', 'QN', 'SN', 'VDD', 'VSS'],
                 'description' : 'D flip-flop with QN output, asynchronous set SN, with drive strength x1'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    DFASQN_D1 = Symsubcircuit(cellInfo['name'], ['D', 'CLK', 'QN', 'SN', 'VDD', 'VSS'])
    # Clock inverters
    ...
    DFASQN_D1.addElement([inv_clkn, inv_clki, invz_in, nand_ml, invz_mfb, invz_ms, nand_sl, invz_sfb, inv_QN])

    # Flatten the circuit
    if genFlat:
        DFASQN_D1_flat = DFASQN_D1.flat()
    if anonimize:
        DFASQN_D1_flat.anonimize()
