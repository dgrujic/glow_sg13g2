...
def info():
    ...
    """
    cellInfo = { 'name' : 'AND3_D1',
                 'pinList' : ['A', 'B', 'C', 'Y', 'VDD', 'VSS'],
                 'description' : 'AND3 with drive strength x1'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    AND3_D1 = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    wn = 420e-9
    wp = 380e-9
    ...
    AND3_D1.addElement([nand3_inst, inv_inst])

    # Flatten the circuit
    if genFlat:
        AND3_D1_flat = AND3_D1.flat()
    if anonimize:
        AND3_D1_flat.anonimize()
