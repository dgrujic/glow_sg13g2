...
def info():
    ...
    """
    cellInfo = { 'name' : 'DFESQQN_D2',
                 'pinList' : ['D', 'E', 'S', 'CLK', 'Q', 'QN', 'VDD', 'VSS'],
                 'description' : 'D flip-flop with enable, set S, and Q and QN outputs with drive strength x2'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    DFESQQN_D2 = Symsubcircuit(cellInfo['name'], ['D', 'E', 'S', 'CLK', 'Q', 'QN', 'VDD', 'VSS'])
    # Clock inverters
    ...
    # Enable inverter
    inv_enb = inv_par("inv_enb", ['E', 'enb', 'VDD', 'VSS'], {'WN' : 150e-9, 'WP' : 300e-9})
    # Input enable network
    # Pull-down network
    ns = SymNMOS("NS", ['net_pdn', 'S', 'VSS', 'VSS'], {'w' : wpdn, 'l' : lmin })
    ...
    # Pull-up network
    ps = SymPMOS("PS", ['nvdd', 'S', 'VDD', 'VDD'], {'w' : wpun, 'l' : lmin })
    p0 = SymPMOS("P0", ['neqi', 'E', 'nvdd', 'VDD'], {'w' : wpun, 'l' : lmin })
    p1 = SymPMOS("P1", ['net_pun', 'qi', 'neqi', 'VDD'], {'w' : wpun, 'l' : lmin })
    p2 = SymPMOS("P2", ['nenbd', 'enb', 'nvdd', 'VDD'], {'w' : wpun, 'l' : lmin })
    p3 = SymPMOS("P3", ['net_pun', 'D', 'nenbd', 'VDD'], {'w' : wpun, 'l' : lmin })
    p4 = SymPMOS("P4", ['dn', 'clki', 'net_pun', 'VDD'], {'w' : wpun, 'l' : lmin })
    # Master latch
    inv_ml = inv_par("inv_ml", ['dn', 'di', 'VDD', 'VSS'], {'WN' : 400e-9, 'WP' : 600e-9})
    invz_mfb = invz_par("invz_mfb", ['di', 'clki', 'clkn', 'dn', 'VDD', 'VSS'], {'WN' : 150e-9, 'WP' : 230e-9, 'WEAK' : 1})
    # Slave latch
    invz1_ms = invz_par("invz1_ms", ['di', 'clki', 'clkn', 'qin', 'VDD', 'VSS'], {'WN' : wnx1, 'WP' : wpx1})
    invz2_ms = invz_par("invz2_ms", ['di', 'clki', 'clkn', 'qin', 'VDD', 'VSS'], {'WN' : wnx1, 'WP' : wpx1})
    inv_sl = inv_par("inv_sl", ['qin', 'qi', 'VDD', 'VSS'], {'WN' : wnx1, 'WP' : wpx1})
    ...
    DFESQQN_D2.addElement([inv_clkn, inv_clki, inv_enb,
                          ns, n0, n1, n2, n3, n4,
                          ps, p0, p1, p2, p3, p4,
                          inv_ml, invz_mfb, 
                          invz1_ms, invz2_ms,
                          inv_sl, invz_sfb, inv_Q, inv_QN])

    # Flatten the circuit
    if genFlat:
        DFESQQN_D2_flat = DFESQQN_D2.flat()
    if anonimize:
        DFESQQN_D2_flat.anonimize()
...
def check(verbose = False):
    ...
    spec = { 'D' : 'D', 'EN' : 'E', 'SET' : 'S', 'Q' : 'Q', 'QN' : 'QN', 'CLK' : 'CLK' }
    res = sim.dffCheck(spec)

    if verbose:
        # Plot waveforms
        wave = sim.filterResults( ['D', 'E', 'S', 'CLK', 'Q', 'QN'] )
