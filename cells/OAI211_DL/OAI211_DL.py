...
def info():
    ...
    """
    cellInfo = { 'name' : 'OAI211_DL',
                 'pinList' : ['A', 'B', 'C', 'D', 'Y', 'VDD', 'VSS'],
                 'description' : 'OAI211 with weak drive strength'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    OAI211_D1 = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])

    n0 = SymNMOS("N0", ['nabp', 'A', 'VSS', 'VSS'], {'w' : wn, 'l' : lmin })
    n1 = SymNMOS("N1", ['nabp', 'B', 'VSS', 'VSS'], {'w' : wn, 'l' : lmin })
    n2 = SymNMOS("N2", ['nabp', 'D', 'ncn', 'VSS'], {'w' : wn, 'l' : lmin })
    n3 = SymNMOS("N3", ['ncn', 'C', 'Y', 'VSS'], {'w' : wn, 'l' : lmin })

    # Pull-up network
    p0 = SymPMOS("P0", ['Y', 'C', 'VDD', 'VDD'], {'w' : wpp, 'l' : lmin })
    p1 = SymPMOS("P1", ['Y', 'D', 'VDD', 'VDD'], {'w' : wpp, 'l' : lmin })
    p2 = SymPMOS("P2", ['nabs', 'A', 'VDD', 'VDD'], {'w' : wp, 'l' : lmin })
    p3 = SymPMOS("P3", ['nabs', 'B', 'Y', 'VDD'], {'w' : wp, 'l' : lmin })

    OAI211_D1.addElement([n0, n1, n2, n3, p0, p1, p2, p3])

    # Flatten the circuit
    if genFlat:
        OAI211_D1_flat = OAI211_D1.flat()
    if anonimize:
        OAI211_D1_flat.anonimize()

def check(verbose = False):
    """
    Check if the circuit works as expected
    """
    expectedFns = [ Not(And(Or(x, y), z, w)) ]
    cellInfo = info()
    name = cellInfo["name"]
