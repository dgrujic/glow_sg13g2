...
def info():
    ...
    """
    cellInfo = { 'name' : 'AO22_D1',
                 'pinList' : ['A', 'B', 'C', 'D', 'Y', 'VDD', 'VSS'],
                 'description' : 'AO22 with drive strength x1'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    AO22_D1 = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    wn = 300e-9
    wp = 400e-9
    ...
    AO22_D1.addElement([n0, n1, n2, n3, p0, p1, p2, p3, inv_inst])

    # Flatten the circuit
    if genFlat:
        AO22_D1_flat = AO22_D1.flat()
    if anonimize:
        AO22_D1_flat.anonimize()
