...
def info():
    ...
    """
    cellInfo = { 'name' : 'DFQN_DL',
                 'pinList' : ['D', 'CLK', 'QN', 'VDD', 'VSS'],
                 'description' : 'D flip-flop with QN output with weak drive strength'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    DFQN_DL = Symsubcircuit("DFQN_DL", ['D', 'CLK', 'QN', 'VDD', 'VSS'])
    # Clock inverters
    ...
    DFQN_DL.addElement([inv_clkn, inv_clki, invz_in, inv_ml, invz_mfb, invz_ms, inv_sl, invz_sfb, inv_QN])

    # Flatten the circuit
    if genFlat:
        DFQN_DL_flat = DFQN_DL.flat()
    if anonimize:
        DFQN_DL_flat.anonimize()
