...
def info():
    ...
    """
    cellInfo = { 'name' : 'DFSQ_D1',
                 'pinList' : ['D', 'CLK', 'S', 'Q', 'VDD', 'VSS'],
                 'description' : 'D flip-flop with synchronous set S, Q output with drive strength x1'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    DFSQ_D1 = Symsubcircuit(cellInfo['name'], ['D', 'CLK', 'S', 'Q', 'VDD', 'VSS'])
    # Clock inverters
    ...
    DFSQ_D1.addElement([inv_clkn, inv_clki, nor_in, invz_in, inv_ml, invz_mfb, invz_ms, inv_sl, invz_sfb, inv_Q])

    # Flatten the circuit
    if genFlat:
        DFSQ_D1_flat = DFSQ_D1.flat()
    if anonimize:
        DFSQ_D1_flat.anonimize()
