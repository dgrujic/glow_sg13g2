...
def info():
    ...
    """
    cellInfo = { 'name' : 'NOR4N1_D2',
                 'pinList' : ['AN', 'B', 'C', 'D', 'Y', 'VDD', 'VSS'],
                 'description' : 'NOR4 with inverted input A and drive strength x2'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    NOR4N1_D2 = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    ...
    NOR4N1_D2.addElement([inv_inst, nor4])

    # Flatten the circuit
    if genFlat:
        NOR4N1_D2_flat = NOR4N1_D2.flat()
    if anonimize:
        NOR4N1_D2_flat.anonimize()
