...
def info():
    ...
    """
    cellInfo = { 'name' : 'DFQN_D1',
                 'pinList' : ['D', 'CLK', 'QN', 'VDD', 'VSS'],
                 'description' : 'D flip-flop with QN output with drive strength x1'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    DFQN_D1 = Symsubcircuit("DFQN_D1", ['D', 'CLK', 'Q', 'QN', 'VDD', 'VSS'])
    # Clock inverters
    ...
    DFQN_D1.addElement([inv_clkn, inv_clki, invz_in, inv_ml, invz_mfb, invz_ms, inv_sl, invz_sfb, inv_QN])

    # Flatten the circuit
    if genFlat:
        DFQN_D1_flat = DFQN_D1.flat()
    if anonimize:
        DFQN_D1_flat.anonimize()
