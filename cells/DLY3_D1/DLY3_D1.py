...
def info():
    ...
    """
    cellInfo = { 'name' : 'DLY3_D1',
                 'pinList' : ['A', 'Y', 'VDD', 'VSS'],
                 'description' : 'Delay x3 with drive strength x1'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    DLY3_D1 = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    ...
    DLY3_D1.addElement([inv1, inv2, inv3, inv4])

    # Flatten the circuit
    if genFlat:
        DLY3_D1_flat = DLY3_D1.flat()
    if anonimize:
        DLY3_D1_flat.anonimize()
