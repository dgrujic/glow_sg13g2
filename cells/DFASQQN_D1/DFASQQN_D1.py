...
def info():
    ...
    """
    cellInfo = { 'name' : 'DFASQQN_D1',
                 'pinList' : ['D', 'CLK', 'Q', 'QN', 'SN', 'VDD', 'VSS'],
                 'description' : 'D flip-flop with Q and QN outputs, asynchronous set SN, with drive strength x1'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    nand_sl = nand2_par("nand_sl", ['SN', 'qin', 'qi', 'VDD', 'VSS'], {'WN' : 300e-9, 'WP' : 330e-9})
    ...
    DFASQQN_D1.addElement([inv_clkn, inv_clki, invz_in, nand_ml, invz_mfb, invz_ms, nand_sl, invz_sfb, inv_qi, inv_Q, inv_QN])

    # Flatten the circuit
    if genFlat:
        DFASQQN_D1_flat = DFASQQN_D1.flat()
    if anonimize:
        DFASQQN_D1_flat.anonimize()
