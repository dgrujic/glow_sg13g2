...
def info():
    ...
    """
    cellInfo = { 'name' : 'CLKBUF_D12',
                 'pinList' : ['A', 'Y', 'VDD', 'VSS'],
                 'description' : 'Clock buffer with drive strength x12'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    CLKBUF_D12 = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    inv1 = inv_par('inv1', ['A', 'AN', 'VDD', 'VSS'], {'WN' : wnin, 'WP' : wpin})
    inv2 = inv_par('inv2', ['AN', 'Y', 'VDD', 'VSS'], {'WN' : wnout, 'WP' : wpout, 'NGN' : ngn, 'NGP' : ngp})
    CLKBUF_D12.addElement([inv1, inv2])

    # Flatten the circuit
    if genFlat:
        CLKBUF_D12_flat = CLKBUF_D12.flat()
    if anonimize:
        CLKBUF_D12_flat.anonimize()
