...
def info():
    ...
    """
    cellInfo = { 'name' : 'MUX3_DL',
                 'pinList' : ['I0', 'I1', 'I2', 'S0', 'S1', 'Y', 'VDD', 'VSS'],
                 'description' : 'MUX3 with weak drive strength'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    MUX3_DL = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    ...
    mux_wn = SymTech.technology['mux3x1WN'] / 2
    mux_wp = 3 * mux_wn / 2
    mux2_wp = SymTech.technology['mux3x1WN'] / 2
    mux2_wn = 2 * mux2_wp / 3 + 30e-9
    tg_wn = SymTech.technology['mux3x1WN'] / 2
    tg_wp = 3 * SymTech.technology['mux3x1WN'] / 4
    ...
    MUX3_DL.addElement([inv_inst_S0, inv_inst_S1, inv_inst_Y])
    MUX3_DL.addElement([invz_inst_I0, invz_inst_I1, invz_inst_I2, tgate_inst])

    # Flatten the circuit
    if genFlat:
        MUX3_DL_flat = MUX3_DL.flat()
    if anonimize:
        MUX3_DL_flat.anonimize()
