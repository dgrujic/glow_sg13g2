...
def info():
    ...
    """
    cellInfo = { 'name' : 'NOR3_D1',
                 'pinList' : ['A', 'B', 'C', 'Y', 'VDD', 'VSS'],
                 'description' : 'NOR3 with drive strength x1'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    NOR3_D1 = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    nor3 = nor3_par('nor3', cellInfo['pinList'])
    NOR3_D1.addElement(nor3)

    # Flatten the circuit
    if genFlat:
        NOR3_D1_flat = NOR3_D1.flat()
    if anonimize:
        NOR3_D1_flat.anonimize()
