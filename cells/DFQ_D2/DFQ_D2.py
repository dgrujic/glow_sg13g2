...
def info():
    ...
    """
    cellInfo = { 'name' : 'DFQ_D2',
                 'pinList' : ['D', 'CLK', 'Q', 'QN', 'VDD', 'VSS'],
                 'description' : 'D flip-flop with Q output with drive strength x2'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    # Master latch
    invz_in = invz_par("invz_in", ['D', 'clkn', 'clki', 'dn', 'VDD', 'VSS'], {'WN' : 200e-9, 'WP' : 300e-9})
    ...
    # Slave latch
    invz_ms = invz_par("invz_ms", ['di', 'clki', 'clkn', 'qin', 'VDD', 'VSS'], {'WN' : 900e-9, 'WP' : 1350e-9})
    inv_sl = inv_par("inv_sl", ['qin', 'qi', 'VDD', 'VSS'], {'WN' : 200e-9, 'WP' : 200e-9})
    invz_sfb = invz_par("invz_sfb", ['qi', 'clkn', 'clki', 'qin', 'VDD', 'VSS'], {'WN' : 150e-9, 'WP' : 150e-9})

    inv_Q = inv_par("inv_Q", ['qin', 'Q', 'VDD', 'VSS'], {'WN' : wn, 'WP' : wp})

    DFQ_D2.addElement([inv_clkn, inv_clki, invz_in, inv_ml, invz_mfb, invz_ms, inv_sl, invz_sfb, inv_Q])

    # Flatten the circuit
    if genFlat:
        DFQ_D2_flat = DFQ_D2.flat()
    if anonimize:
        DFQ_D2_flat.anonimize()
