...
def info():
    ...
    """
    cellInfo = { 'name' : 'NOR4N2_D1',
                 'pinList' : ['AN', 'BN', 'C', 'D', 'Y', 'VDD', 'VSS'],
                 'description' : 'NOR4 with inverted inputs A, B and drive strength x1'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    NOR4N2_D1 = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    ...
    NOR4N2_D1.addElement([inv_inst1, inv_inst2, nor4])

    # Flatten the circuit
    if genFlat:
        NOR4N2_D1_flat = NOR4N2_D1.flat()
    if anonimize:
        NOR4N2_D1_flat.anonimize()
