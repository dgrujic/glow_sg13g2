...
def info():
    ...
    """
    cellInfo = { 'name' : 'NOR4_D4',
                 'pinList' : ['A', 'B', 'C', 'D', 'Y', 'VDD', 'VSS'],
                 'description' : 'NOR4 with drive strength x4'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    ngp = 3
    ngn = 2
    NOR4_D4 = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    nor4 = nor4_par('nor4', cellInfo['pinList'], {'WP' : wp, 'NGP' : ngp, 'WN' : wn, 'NGN' : ngn})
    NOR4_D4.addElement(nor4)

    # Flatten the circuit
    if genFlat:
        NOR4_D4_flat = NOR4_D4.flat()
    if anonimize:
        NOR4_D4_flat.anonimize()
