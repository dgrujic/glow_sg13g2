...
def info():
    ...
    """
    cellInfo = { 'name' : 'CLKINV_D2',
                 'pinList' : ['A', 'Y', 'VDD', 'VSS'],
                 'description' : 'Clock inverter with drive strength x2'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    CLKINV_D2 = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    inv1 = inv_par('inv1', ['A', 'Y', 'VDD', 'VSS'], {'WN' : wn, 'WP' : wp})
    CLKINV_D2.addElement(inv1)

    # Flatten the circuit
    if genFlat:
        CLKINV_D2_flat = CLKINV_D2.flat()
    if anonimize:
        CLKINV_D2_flat.anonimize()
