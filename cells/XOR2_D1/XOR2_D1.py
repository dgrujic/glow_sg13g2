...
def info():
    ...
    """
    cellInfo = { 'name' : 'XOR2_D1',
                 'pinList' : ['A', 'B', 'Y', 'VDD', 'VSS'],
                 'description' : 'XOR2 with drive strength x1'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    XOR2_D1 = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    ...
    # NMOS
    n0 = SymNMOS("N0", ['BN', 'A', 'Y', 'VSS'], {'w' : wn, 'l' : lmin })
    n1 = SymNMOS("N1", ['BI', 'AN', 'Y', 'VSS'], {'w' : wn, 'l' : lmin })

    # PMOS
    p0 = SymPMOS("P0", ['BN', 'AN', 'Y', 'VDD'], {'w' : wp, 'l' : lmin })
    p1 = SymPMOS("P1", ['BI', 'A', 'Y', 'VDD'], {'w' : wp, 'l' : lmin })
    ...
    XOR2_D1.addElement([inv_inst1, inv_inst2, inv_inst3, n0, n1, p0, p1])

    # Flatten the circuit
    if genFlat:
        XOR2_D1_flat = XOR2_D1.flat()
    if anonimize:
        XOR2_D1_flat.anonimize()
