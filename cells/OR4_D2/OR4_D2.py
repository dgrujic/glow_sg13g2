...
def info():
    ...
    """
    cellInfo = { 'name' : 'OR4_D2',
                 'pinList' : ['A', 'B', 'C', 'D', 'Y', 'VDD', 'VSS'],
                 'description' : 'OR4 with drive strength x2'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    OR4_D2 = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    wn = 500e-9
    wp = 1300e-9
    ...
    OR4_D2.addElement([nor4_inst, inv_inst])

    # Flatten the circuit
    if genFlat:
        OR4_D2_flat = OR4_D2.flat()
    if anonimize:
        OR4_D2_flat.anonimize()
