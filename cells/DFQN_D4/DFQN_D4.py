...
def info():
    ...
    """
    cellInfo = { 'name' : 'DFQN_D4',
                 'pinList' : ['D', 'CLK', 'QN', 'VDD', 'VSS'],
                 'description' : 'D flip-flop with QN output with drive strength x4'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    DFQN_D4.addElement([inv_clkn, inv_clki, invz_in, inv_ml, invz_mfb, invz_ms, inv_sl, invz_sfb, inv_QN])

    # Flatten the circuit
    if genFlat:
        DFQN_D4_flat = DFQN_D4.flat()
    if anonimize:
        DFQN_D4_flat.anonimize()
