...
def info():
    ...
    """
    cellInfo = { 'name' : 'OA22_DL',
                 'pinList' : ['A', 'B', 'C', 'D', 'Y', 'VDD', 'VSS'],
                 'description' : 'OA22 with weak drive strength'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    OA22_DL = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    ...
    OA22_DL.addElement([n0, n1, n2, n3, p0, p1, p2, p3, inv1])

    # Flatten the circuit
    if genFlat:
        OA22_DL_flat = OA22_DL.flat()
    if anonimize:
        OA22_DL_flat.anonimize()
