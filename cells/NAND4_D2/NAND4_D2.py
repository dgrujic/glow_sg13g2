...
def generate(genFlat = True, anonimize = True):
    ...
    NAND4_D2 = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    nand4 = nand4_par('nand4', cellInfo['pinList'], {'WN' : wn, 'WP' : wp, 'NGN' : ngn, 'NGP' : ngp })
    NAND4_D2.addElement(nand4)

    # Flatten the circuit
    if genFlat:
        NAND4_D2_flat = NAND4_D2.flat()
    if anonimize:
        NAND4_D2_flat.anonimize()
