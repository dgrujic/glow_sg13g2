...
def info():
    ...
    """
    cellInfo = { 'name' : 'OR2_D4',
                 'pinList' : ['A', 'B', 'Y', 'VDD', 'VSS'],
                 'description' : 'OR2 with drive strength x4'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    OR2_D4 = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    wn = 800e-9
    wp = 1500e-9
    ...
    OR2_D4.addElement([nor2_inst, inv_inst])

    # Flatten the circuit
    if genFlat:
        OR2_D4_flat = OR2_D4.flat()
    if anonimize:
        OR2_D4_flat.anonimize()
