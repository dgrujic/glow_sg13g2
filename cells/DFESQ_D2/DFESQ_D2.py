...
def info():
    ...
    """
    cellInfo = { 'name' : 'DFESQ_D2',
                 'pinList' : ['D', 'E', 'S', 'CLK', 'Q', 'VDD', 'VSS'],
                 'description' : 'D flip-flop with enable, set S, and Q output with drive strength x2'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    DFESQ_D2 = Symsubcircuit(cellInfo['name'], ['D', 'E', 'S', 'CLK', 'Q', 'VDD', 'VSS'])
    # Clock inverters
    ...
    DFESQ_D2.addElement([inv_clkn, inv_clki, inv_enb,
                          ns, n0, n1, n2, n3, n4,
                          ps, p0, p1, p2, p3, p4,
                          inv_ml, invz_mfb, 
                          invz1_ms, invz2_ms,
    ...
    # Flatten the circuit
    if genFlat:
        DFESQ_D2_flat = DFESQ_D2.flat()
    if anonimize:
        DFESQ_D2_flat.anonimize()
...
def check(verbose = False):
    ...
    spec = { 'D' : 'D', 'EN' : 'E', 'SET' : 'S', 'Q' : 'Q', 'CLK' : 'CLK' }
    res = sim.dffCheck(spec)

    if verbose:
        # Plot waveforms
        wave = sim.filterResults( ['D', 'E', 'S', 'CLK', 'Q'] )
