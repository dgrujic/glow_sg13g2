...
def info():
    ...
    """
    cellInfo = { 'name' : 'DLY4_D4',
                 'pinList' : ['A', 'Y', 'VDD', 'VSS'],
                 'description' : 'Delay x4 with drive strength x4'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    wnout = wpout / 3
    lch = 4 * SymTech.technology['Lmin']
    ng = 2

    DLY4_D4 = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    ...
    DLY4_D4.addElement([inv1, inv2, inv3, inv4])

    # Flatten the circuit
    if genFlat:
        DLY4_D4_flat = DLY4_D4.flat()
    if anonimize:
        DLY4_D4_flat.anonimize()
