...
def info():
    ...
    """
    cellInfo = { 'name' : 'DFSQQN_D1',
                 'pinList' : ['D', 'CLK', 'S', 'Q', 'QN', 'VDD', 'VSS'],
                 'description' : 'D flip-flop with synchronous set S, Q and QN outputs with drive strength x1'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    DFSQQN_D1 = Symsubcircuit(cellInfo['name'], ['D', 'CLK', 'S', 'Q', 'QN', 'VDD', 'VSS'])
    # Clock inverters
    ...
    # Reset
    nor_in = nor2_par("nor_in", ['S', 'D', 'dnor', 'VDD', 'VSS', ], {'WN' : 150e-9, 'WP' : 300e-9})
    # Master latch
    ...
    DFSQQN_D1.addElement([inv_clkn, inv_clki, nor_in, invz_in, inv_ml, invz_mfb, invz_ms, inv_sl, invz_sfb, inv_Q, inv_QN])

    # Flatten the circuit
    if genFlat:
        DFSQQN_D1_flat = DFSQQN_D1.flat()
    if anonimize:
        DFSQQN_D1_flat.anonimize()
