...
def info():
    ...
    """
    cellInfo = { 'name' : 'CLKINV_D1',
                 'pinList' : ['A', 'Y', 'VDD', 'VSS'],
                 'description' : 'Clock inverter with drive strength x1'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    CLKINV_D1 = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    inv1 = inv_par('inv1', ['A', 'Y', 'VDD', 'VSS'], {'WN' : wn, 'WP' : wp})
    CLKINV_D1.addElement(inv1)

    # Flatten the circuit
    if genFlat:
        CLKINV_D1_flat = CLKINV_D1.flat()
    if anonimize:
        CLKINV_D1_flat.anonimize()
