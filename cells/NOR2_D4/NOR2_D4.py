...
def info():
    ...
    """
    cellInfo = { 'name' : 'NOR2_D4',
                 'pinList' : ['A', 'B', 'Y', 'VDD', 'VSS'],
                 'description' : 'NOR2 with drive strength x4'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    NOR2_D4 = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    ...
    NOR2_D4.addElement([nor2_pdn1, nor2_pun1, nor2_pun2, nor2_pun3])
    
    # Flatten the circuit
    if genFlat:
        NOR2_D4_flat = NOR2_D4.flat()
    if anonimize:
        NOR2_D4_flat.anonimize()
