...
def info():
    ...
    """
    cellInfo = { 'name' : 'NOR4N1_D1',
                 'pinList' : ['AN', 'B', 'C', 'D', 'Y', 'VDD', 'VSS'],
                 'description' : 'NOR4 with inverted input A and drive strength x1'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    NOR4N1_D1 = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    nor4_pins = nor4_par.getTerminals()
    nor4 = nor4_par('nor4', nor4_pins)
    inv_wn = 200e-9
    inv_wp = 300e-9
    inv_inst = inv_par('inv_inst', ['AN', 'A', 'VDD', 'VSS'], {'WN' : inv_wn, 'WP' : inv_wp})
    NOR4N1_D1.addElement([inv_inst, nor4])

    # Flatten the circuit
    if genFlat:
        NOR4N1_D1_flat = NOR4N1_D1.flat()
    if anonimize:
        NOR4N1_D1_flat.anonimize()
