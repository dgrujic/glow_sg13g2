...
def info():
    ...
    """
    cellInfo = { 'name' : 'NOR4N1_DL',
                 'pinList' : ['AN', 'B', 'C', 'D', 'Y', 'VDD', 'VSS'],
                 'description' : 'NOR4 with inverted input A and weak drive strength'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    NOR4N1_DL = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    ...
    NOR4N1_DL.addElement([inv_inst, nor4])

    # Flatten the circuit
    if genFlat:
        NOR4N1_DL_flat = NOR4N1_DL.flat()
    if anonimize:
        NOR4N1_DL_flat.anonimize()
