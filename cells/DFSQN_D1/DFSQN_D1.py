...
def info():
    ...
    """
    cellInfo = { 'name' : 'DFSQN_D1',
                 'pinList' : ['D', 'CLK', 'S', 'QN', 'VDD', 'VSS'],
                 'description' : 'D flip-flop with synchronous set S, QN output with drive strength x1'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    DFSQN_D1 = Symsubcircuit(cellInfo['name'], ['D', 'CLK', 'S', 'QN', 'VDD', 'VSS'])
    # Clock inverters
    ...
    DFSQN_D1.addElement([inv_clkn, inv_clki, nor_in, invz_in, inv_ml, invz_mfb, invz_ms, inv_sl, invz_sfb, inv_QN])

    # Flatten the circuit
    if genFlat:
        DFSQN_D1_flat = DFSQN_D1.flat()
    if anonimize:
        DFSQN_D1_flat.anonimize()
