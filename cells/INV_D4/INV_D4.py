...
def info():
    ...
    """
    cellInfo = { 'name' : 'INV_D4',
                 'pinList' : ['A', 'Y', 'VDD', 'VSS'],
                 'description' : 'Inverter with drive strength x4'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    INV_D4 = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    inv4 = inv_par('inv4', ['A', 'Y', 'VDD', 'VSS'], {'WN' : wn, 'WP' : wp, 'NGN' : ngates, 'NGP' : ngates})
    INV_D4.addElement(inv4)

    # Flatten the circuit
    if genFlat:
        INV_D4_flat = INV_D4.flat()
    if anonimize:
        INV_D4_flat.anonimize()
