...
def info():
    ...
    """
    cellInfo = { 'name' : 'DFQ_D1',
                 'pinList' : ['D', 'CLK', 'Q', 'QN', 'VDD', 'VSS'],
                 'description' : 'D flip-flop with Q output with drive strength x1'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    DFQ_D1 = Symsubcircuit("DFQ_D1", ['D', 'CLK', 'Q', 'QN', 'VDD', 'VSS'])
    # Clock inverters
    ...
    DFQ_D1.addElement([inv_clkn, inv_clki, invz_in, inv_ml, invz_mfb, invz_ms, inv_sl, invz_sfb, inv_Q])

    # Flatten the circuit
    if genFlat:
        DFQ_D1_flat = DFQ_D1.flat()
    if anonimize:
        DFQ_D1_flat.anonimize()
