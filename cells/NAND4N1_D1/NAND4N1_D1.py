...
def info():
    ...
    """
    cellInfo = { 'name' : 'NAND4N1_D1',
                 'pinList' : ['AN', 'B', 'C', 'D', 'Y', 'VDD', 'VSS'],
                 'description' : 'NAND4 with inverted input A and drive strength x1'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    NAND4N1_D1 = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    nand_name = "NAND4_D1"
    ...
    NAND4N1_D1.addElement([nand4_inst, inv_inst])

    # Flatten the circuit
    if genFlat:
        NAND4N1_D1_flat = NAND4N1_D1.flat()
    if anonimize:
        NAND4N1_D1_flat.anonimize()
