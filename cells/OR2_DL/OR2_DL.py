...
def info():
    ...
    """
    cellInfo = { 'name' : 'OR2_DL',
                 'pinList' : ['A', 'B', 'Y', 'VDD', 'VSS'],
                 'description' : 'OR2 with weak drive strength'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    OR2_DL = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    wn = 150e-9
    wp = 2 * wn
    ...
    OR2_DL.addElement([nor2_inst, inv_inst])

    # Flatten the circuit
    if genFlat:
        OR2_DL_flat = OR2_DL.flat()
    if anonimize:
        OR2_DL_flat.anonimize()
