...
def info():
    ...
    """
    cellInfo = { 'name' : 'NAND4_DL',
                 'pinList' : ['A', 'B', 'C', 'D', 'Y', 'VDD', 'VSS'],
                 'description' : 'NAND4 with weak drive strength'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    NAND4_DL = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    nand4 = nand4_par('nand4', cellInfo['pinList'], {'WN' : wn, 'WP' : wp})
    NAND4_DL.addElement(nand4)

    # Flatten the circuit
    if genFlat:
        NAND4_DL_flat = NAND4_DL.flat()
    if anonimize:
        NAND4_DL_flat.anonimize()
