...
def info():
    ...
    """
    cellInfo = { 'name' : 'DFRQ_D1',
                 'pinList' : ['D', 'CLK', 'RN', 'Q', 'VDD', 'VSS'],
                 'description' : 'D flip-flop with synchronous reset RN, Q output with drive strength x1'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    DFRQ_D1 = Symsubcircuit(cellInfo['name'], ['D', 'CLK', 'RN', 'Q', 'VDD', 'VSS'])
    # Clock inverters
    ...
    DFRQ_D1.addElement([inv_clkn, inv_clki, nand_in, invz_in, inv_ml, invz_mfb, invz_ms, inv_sl, invz_sfb, inv_Q])

    # Flatten the circuit
    if genFlat:
        DFRQ_D1_flat = DFRQ_D1.flat()
    if anonimize:
        DFRQ_D1_flat.anonimize()
