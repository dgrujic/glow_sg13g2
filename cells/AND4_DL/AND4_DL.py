...
def info():
    ...
    """
    cellInfo = { 'name' : 'AND4_DL',
                 'pinList' : ['A', 'B', 'C', 'D', 'Y', 'VDD', 'VSS'],
                 'description' : 'AND4 with weak drive strength'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    AND4_DL = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    wn = 340e-9
    wp = 300e-9
    ...
    AND4_DL.addElement([nand4_inst, inv_inst])

    # Flatten the circuit
    if genFlat:
        AND4_DL_flat = AND4_DL.flat()
    if anonimize:
        AND4_DL_flat.anonimize()
