...
def info():
    ...
    """
    cellInfo = { 'name' : 'DFRQN_D1',
                 'pinList' : ['D', 'CLK', 'RN', 'QN', 'VDD', 'VSS'],
                 'description' : 'D flip-flop with synchronous reset RN, QN output with drive strength x1'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    DFRQN_D1 = Symsubcircuit(cellInfo['name'], ['D', 'CLK', 'RN', 'QN', 'VDD', 'VSS'])
    # Clock inverters
    ...
    DFRQN_D1.addElement([inv_clkn, inv_clki, nand_in, invz_in, inv_ml, invz_mfb, invz_ms, inv_sl, invz_sfb, inv_QN])

    # Flatten the circuit
    if genFlat:
        DFRQN_D1_flat = DFRQN_D1.flat()
    if anonimize:
        DFRQN_D1_flat.anonimize()
