...
def info():
    ...
    """
    cellInfo = { 'name' : 'AO21_D2',
                 'pinList' : ['A', 'B', 'C', 'Y', 'VDD', 'VSS'],
                 'description' : 'AO21 with drive strength x2'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    AO21_D2 = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    wns = 500e-9
    wnp = 300e-9
    wp = 600e-9
    ...
    AO21_D2.addElement([n0, n1, n2, p0, p1, p2, inv_inst])

    # Flatten the circuit
    if genFlat:
        AO21_D2_flat = AO21_D2.flat()
    if anonimize:
        AO21_D2_flat.anonimize()
