...
def info():
    ...
    """
    cellInfo = { 'name' : 'NAND2_D1',
                 'pinList' : ['A', 'B', 'Y', 'VDD', 'VSS'],
                 'description' : 'NAND2 with drive strength x1'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    NAND2_D1 = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    nand2 = nand2_par('nand2', cellInfo['pinList'], {'WN' : 900e-9, 'WP' : wp})
    NAND2_D1.addElement(nand2)

    # Flatten the circuit
    if genFlat:
        NAND2_D1_flat = NAND2_D1.flat()
    if anonimize:
        NAND2_D1_flat.anonimize()
