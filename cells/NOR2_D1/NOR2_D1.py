...
def info():
    ...
    """
    cellInfo = { 'name' : 'NOR2_D1',
                 'pinList' : ['A', 'B', 'Y', 'VDD', 'VSS'],
                 'description' : 'NOR2 with drive strength x1'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    NOR2_D1 = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    nor2 = nor2_par('nor2', cellInfo['pinList'])
    NOR2_D1.addElement(nor2)

    # Flatten the circuit
    if genFlat:
        NOR2_D1_flat = NOR2_D1.flat()
    if anonimize:
        NOR2_D1_flat.anonimize()
