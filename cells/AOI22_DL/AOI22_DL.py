...
def info():
    ...
    """
    cellInfo = { 'name' : 'AOI22_DL',
                 'pinList' : ['A', 'B', 'C', 'D', 'Y', 'VDD', 'VSS'],
                 'description' : 'AOI22 with weak drive strength'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    AOI22_DL = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    wn = 500e-9
    wp = 3 * wn / 2
    ...
    AOI22_DL.addElement([n0, n1, n2, n3, p0, p1, p2, p3])

    # Flatten the circuit
    if genFlat:
        AOI22_DL_flat = AOI22_DL.flat()
    if anonimize:
        AOI22_DL_flat.anonimize()
