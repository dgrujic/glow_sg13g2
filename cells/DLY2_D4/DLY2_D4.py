...
def info():
    ...
    """
    cellInfo = { 'name' : 'DLY2_D4',
                 'pinList' : ['A', 'Y', 'VDD', 'VSS'],
                 'description' : 'Delay x2 with drive strength x4'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    DLY2_D4 = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    ...
    DLY2_D4.addElement([inv1, inv2, inv3, inv4])

    # Flatten the circuit
    if genFlat:
        DLY2_D4_flat = DLY2_D4.flat()
    if anonimize:
        DLY2_D4_flat.anonimize()
