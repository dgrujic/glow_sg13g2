...
def info():
    ...
    """
    cellInfo = { 'name' : 'OA21_D4',
                 'pinList' : ['A', 'B', 'C', 'Y', 'VDD', 'VSS'],
                 'description' : 'OA21 with drive strength x4'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    ng = 2
    wninv = ng * SymTech.technology['invx2WN']
    wpinv = ng * SymTech.technology['invx2WP']
    
    lmin = SymTech.technology['Lmin']

    OA21_D4 = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    ...
    inv1 = inv_par('inv1', ['NY', 'Y', 'VDD', 'VSS'], {'WN' : wninv, 'WP' : wpinv, 'NGN' : ng, 'NGN' : ng})
    OA21_D4.addElement([n0, n1, n2, p0, p1, p2, inv1])

    # Flatten the circuit
    if genFlat:
        OA21_D4_flat = OA21_D4.flat()
    if anonimize:
        OA21_D4_flat.anonimize()
