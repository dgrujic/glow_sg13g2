...
def info():
    ...
    """
    cellInfo = { 'name' : 'OA21_DL',
                 'pinList' : ['A', 'B', 'C', 'Y', 'VDD', 'VSS'],
                 'description' : 'OA21 with weak drive strength'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    wp = 1.5 * wn
    wpp = 250e-9
    wninv = SymTech.technology['invx1WN'] / 2
    wpinv = SymTech.technology['invx1WP'] / 2
    lmin = SymTech.technology['Lmin']

    OA21_DL = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    ...
    OA21_DL.addElement([n0, n1, n2, p0, p1, p2, inv1])

    # Flatten the circuit
    if genFlat:
        OA21_DL_flat = OA21_DL.flat()
    if anonimize:
        OA21_DL_flat.anonimize()
