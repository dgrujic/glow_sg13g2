...
def info():
    ...
    """
    cellInfo = { 'name' : 'OAI21_DL',
                 'pinList' : ['A', 'B', 'C', 'Y', 'VDD', 'VSS'],
                 'description' : 'OAI21 with weak drive strength'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    OAI21_DL = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    ...
    OAI21_DL.addElement([n0, n1, n2, p0, p1, p2])

    # Flatten the circuit
    if genFlat:
        OAI21_DL_flat = OAI21_DL.flat()
    if anonimize:
        OAI21_DL_flat.anonimize()
