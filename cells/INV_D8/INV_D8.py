...
def info():
    ...
    """
    cellInfo = { 'name' : 'INV_D8',
                 'pinList' : ['A', 'Y', 'VDD', 'VSS'],
                 'description' : 'Inverter with drive strength x8'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    INV_D8 = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    inv8 = inv_par('inv8', ['A', 'Y', 'VDD', 'VSS'], {'WN' : wn, 'WP' : wp, 'NGN' : ngates, 'NGP' : ngates})
    INV_D8.addElement(inv8)

    # Flatten the circuit
    if genFlat:
        INV_D8_flat = INV_D8.flat()
    if anonimize:
        INV_D8_flat.anonimize()
