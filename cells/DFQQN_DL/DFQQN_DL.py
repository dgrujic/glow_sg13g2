...
def info():
    ...
    """
    cellInfo = { 'name' : 'DFQQN_DL',
                 'pinList' : ['D', 'CLK', 'Q', 'QN', 'VDD', 'VSS'],
                 'description' : 'D flip-flop with Q and QN outputs with weak drive strength'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    DFQQN_DL = Symsubcircuit("DFQQN_DL", ['D', 'CLK', 'Q', 'QN', 'VDD', 'VSS'])
    # Clock inverters
    ...
    DFQQN_DL.addElement([inv_clkn, inv_clki, invz_in, inv_ml, invz_mfb, invz_ms, inv_sl, invz_sfb, inv_Q, inv_QN])

    # Flatten the circuit
    if genFlat:
        DFQQN_DL_flat = DFQQN_DL.flat()
    if anonimize:
        DFQQN_DL_flat.anonimize()
