...
def info():
    ...
    """
    cellInfo = { 'name' : 'AND4_D2',
                 'pinList' : ['A', 'B', 'C', 'D', 'Y', 'VDD', 'VSS'],
                 'description' : 'AND4 with drive strength x2'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    AND4_D2 = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    wn = 750e-9
    wp = 650e-9
    ...
    AND4_D2.addElement([nand4_inst, inv_inst])

    # Flatten the circuit
    if genFlat:
        AND4_D2_flat = AND4_D2.flat()
    if anonimize:
        AND4_D2_flat.anonimize()
