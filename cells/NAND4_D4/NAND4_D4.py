...
def info():
    ...
    """
    cellInfo = { 'name' : 'NAND4_D4',
                 'pinList' : ['A', 'B', 'C', 'D', 'Y', 'VDD', 'VSS'],
                 'description' : 'NAND2 with drive strength x4'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    NAND4_D4 = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    nand4 = nand4_par('nand4', cellInfo['pinList'], {'WN' : wn, 'WP' : wp, 'NGN' : ngn, 'NGP' : ngp })
    NAND4_D4.addElement(nand4)

    # Flatten the circuit
    if genFlat:
        NAND4_D4_flat = NAND4_D4.flat()
    if anonimize:
        NAND4_D4_flat.anonimize()
