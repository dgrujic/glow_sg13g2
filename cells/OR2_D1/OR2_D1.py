...
def info():
    ...
    """
    cellInfo = { 'name' : 'OR2_D1',
                 'pinList' : ['A', 'B', 'Y', 'VDD', 'VSS'],
                 'description' : 'OR2 with drive strength x1'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    OR2_D1 = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    wn = 200e-9
    wp = 2 * wn
    nor2_inst = nor2_par('nor2', ['A', 'B', 'NO', 'VDD', 'VSS'], {'WN' : wn, 'WP' : wp})
    ...
    OR2_D1.addElement([nor2_inst, inv_inst])

    # Flatten the circuit
    if genFlat:
        OR2_D1_flat = OR2_D1.flat()
    if anonimize:
        OR2_D1_flat.anonimize()
