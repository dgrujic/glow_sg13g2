...
def info():
    ...
    """
    cellInfo = { 'name' : 'DFQ_DL',
                 'pinList' : ['D', 'CLK', 'Q', 'QN', 'VDD', 'VSS'],
                 'description' : 'D flip-flop with Q output with weak drive strength'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    DFQ_DL = Symsubcircuit("DFQ_DL", ['D', 'CLK', 'Q', 'QN', 'VDD', 'VSS'])
    # Clock inverters
    ...
    DFQ_DL.addElement([inv_clkn, inv_clki, invz_in, inv_ml, invz_mfb, invz_ms, inv_sl, invz_sfb, inv_Q])

    # Flatten the circuit
    if genFlat:
        DFQ_DL_flat = DFQ_DL.flat()
    if anonimize:
        DFQ_DL_flat.anonimize()
