...
def info():
    ...
    """
    cellInfo = { 'name' : 'AOI211_DL',
                 'pinList' : ['A', 'B', 'C', 'D', 'Y', 'VDD', 'VSS'],
                 'description' : 'AOI211 with weak drive strength'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    AOI211_DL = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    wnp = 300e-9
    wns = 1.5 * wnp
    wp = 5 * wnp / 2
    ...
    AOI211_DL.addElement([n0, n1, n2, n3, p0, p1, p2, p3])

    # Flatten the circuit
    if genFlat:
        AOI211_DL_flat = AOI211_DL.flat()
    if anonimize:
        AOI211_DL_flat.anonimize()
