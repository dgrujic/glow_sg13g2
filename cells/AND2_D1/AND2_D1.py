...
def info():
    ...
    """
    cellInfo = { 'name' : 'AND2_D1',
                 'pinList' : ['A', 'B', 'Y', 'VDD', 'VSS'],
                 'description' : 'AND2 with drive strength x1'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    AND2_D1 = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    wn = 320e-9
    wp = 360e-9
    ...
    AND2_D1.addElement([nand2_inst, inv_inst])

    # Flatten the circuit
    if genFlat:
        AND2_D1_flat = AND2_D1.flat()
    if anonimize:
        AND2_D1_flat.anonimize()
