...
def info():
    ...
    """
    cellInfo = { 'name' : 'DLY4_D1',
                 'pinList' : ['A', 'Y', 'VDD', 'VSS'],
                 'description' : 'Delay x4 with drive strength x1'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    DLY4_D1 = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    ...
    DLY4_D1.addElement([inv1, inv2, inv3, inv4])

    # Flatten the circuit
    if genFlat:
        DLY4_D1_flat = DLY4_D1.flat()
    if anonimize:
        DLY4_D1_flat.anonimize()
