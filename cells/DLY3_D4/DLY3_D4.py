...
def info():
    ...
    """
    cellInfo = { 'name' : 'DLY3_D4',
                 'pinList' : ['A', 'Y', 'VDD', 'VSS'],
                 'description' : 'Delay x3 with drive strength x4'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    wnout = wpout / 3
    lch = 3 * SymTech.technology['Lmin']
    ng = 2

    DLY3_D4 = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    ...
    DLY3_D4.addElement([inv1, inv2, inv3, inv4])

    # Flatten the circuit
    if genFlat:
        DLY3_D4_flat = DLY3_D4.flat()
    if anonimize:
        DLY3_D4_flat.anonimize()
