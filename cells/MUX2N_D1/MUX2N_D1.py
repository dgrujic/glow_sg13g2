...
def info():
    ...
    """
    cellInfo = { 'name' : 'MUX2N_D1',
                 'pinList' : ['I0', 'I1', 'S0', 'YN', 'VDD', 'VSS'],
                 'description' : 'MUX2 with inverted output and drive strength x1'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    MUX2N_D1 = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    inv_min_wn = 240e-9
    inv_min_wp = 3 * inv_min_wn / 2
    inv_inst_S0 = inv_par('inv_inst_S0', ['S0', 'S0B', 'VDD', 'VSS'], {'WN' : inv_min_wn, 'WP' : inv_min_wp})

    mux_wn = SymTech.technology['invx1WN']
    mux_wp = 3 * mux_wn / 2
    ...
    MUX2N_D1.addElement([inv_inst_S0, invz_inst_I0, invz_inst_I1])

    # Flatten the circuit
    if genFlat:
        MUX2N_D1_flat = MUX2N_D1.flat()
    if anonimize:
        MUX2N_D1_flat.anonimize()
