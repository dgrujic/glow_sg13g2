...
def info():
    ...
    """
    cellInfo = { 'name' : 'OR3_D2',
                 'pinList' : ['A', 'B', 'C', 'Y', 'VDD', 'VSS'],
                 'description' : 'OR3 with drive strength x2'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    OR3_D2 = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    wn = 500e-9
    wp = 1100e-9
    ...
    OR3_D2.addElement([nor3_inst, inv_inst])

    # Flatten the circuit
    if genFlat:
        OR3_D2_flat = OR3_D2.flat()
    if anonimize:
        OR3_D2_flat.anonimize()
