...
def info():
    ...
    """
    cellInfo = { 'name' : 'OA21_D1',
                 'pinList' : ['A', 'B', 'C', 'Y', 'VDD', 'VSS'],
                 'description' : 'OA21 with drive strength x1'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    wp = 1.5 * wn
    wpp = 350e-9
    wninv = SymTech.technology['invx1WN']
    wpinv = SymTech.technology['invx1WP']
    lmin = SymTech.technology['Lmin']

    OA21_D1 = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    ...
    OA21_D1.addElement([n0, n1, n2, p0, p1, p2, inv1])

    # Flatten the circuit
    if genFlat:
        OA21_D1_flat = OA21_D1.flat()
    if anonimize:
        OA21_D1_flat.anonimize()
