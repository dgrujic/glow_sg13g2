...
def info():
    ...
    """
    cellInfo = { 'name' : 'NOR4_D1',
                 'pinList' : ['A', 'B', 'C', 'D', 'Y', 'VDD', 'VSS'],
                 'description' : 'NOR4 with drive strength x1'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    NOR4_D1 = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    nor4 = nor4_par('nor4', cellInfo['pinList'])
    NOR4_D1.addElement(nor4)

    # Flatten the circuit
    if genFlat:
        NOR4_D1_flat = NOR4_D1.flat()
    if anonimize:
        NOR4_D1_flat.anonimize()
