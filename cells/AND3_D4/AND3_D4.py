...
def info():
    ...
    """
    cellInfo = { 'name' : 'AND3_D4',
                 'pinList' : ['A', 'B', 'C', 'Y', 'VDD', 'VSS'],
                 'description' : 'AND3 with drive strength x4'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    AND3_D4 = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    wn = 1000e-9
    wp = 800e-9
    ...
    AND3_D4.addElement([nand3_inst, inv_inst])

    # Flatten the circuit
    if genFlat:
        AND3_D4_flat = AND3_D4.flat()
    if anonimize:
        AND3_D4_flat.anonimize()
