...
def info():
    ...
    """
    cellInfo = { 'name' : 'AND3_D2',
                 'pinList' : ['A', 'B', 'C', 'Y', 'VDD', 'VSS'],
                 'description' : 'AND3 with drive strength x2'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    AND3_D2 = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    wn = 500e-9
    wp = 450e-9
    ...
    AND3_D2.addElement([nand3_inst, inv_inst])

    # Flatten the circuit
    if genFlat:
        AND3_D2_flat = AND3_D2.flat()
    if anonimize:
        AND3_D2_flat.anonimize()
