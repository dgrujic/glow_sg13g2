...
def info():
    ...
    """
    cellInfo = { 'name' : 'INV_D14',
                 'pinList' : ['A', 'Y', 'VDD', 'VSS'],
                 'description' : 'Inverter with drive strength x16'
    }
    return cellInfo
...
def generate(genFlat = True, anonimize = True):
    ...
    INV_D16 = Symsubcircuit(cellInfo['name'], cellInfo['pinList'])
    inv16 = inv_par('inv16', ['A', 'Y', 'VDD', 'VSS'], {'WN' : wn, 'WP' : wp, 'NGN' : ngates, 'NGP' : ngates})
    INV_D16.addElement(inv16)

    # Flatten the circuit
    if genFlat:
        INV_D16_flat = INV_D16.flat()
    if anonimize:
        INV_D16_flat.anonimize()
